...
export const $from = combine($wallets, $idx, (wallets, idx) => wallets[idx]);
export const $to = combine($savings, $idx, (savings, idx) => savings[idx]);
...
// $payload.watch((s) => logline('[transferToSaving] payload', s));

export const pressSubmit = TransferDomain.createEvent<React.SyntheticEvent>();
export const transferToSavingRequest = TransferDomain.createEvent<
  TransferPayload
>();

export const transferToSavingRequestFx = TransferDomain.createEffect<
  TransferPayload,
...
export const redirectToTransferToSaving = TransferDomain.createEvent<string>();

export const redirectToTransferToSavingFx = TransferDomain.createEffect();
